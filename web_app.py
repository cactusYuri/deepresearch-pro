...
from research_assistant.tasks import  start_new_task,continue_task,get_task_results
from research_assistant.schemas import Task, task_storage
...
def generate_task_id(topic):
    return str(uuid.uuid4())

def generate_task_id2(topic: str) -> str:
...
# API Routes converted from FastAPI to Flask
...
def submit_new_task():
    ...
    try:
        ...
        selected_conferences = data.get('selected_conferences', [])
        ...
        # Here you would typically call your LLM interface for clarification
        ...
        def run_async_task():
            ...
            try:
                loop.run_until_complete(start_new_task(task_id))
            finally:
                loop.close()
...
@app.route('/api/v1/tasks/<task_id>/confirm', methods=['POST'])
def confirm_task(task_id):
    ...
    try:
        ...
        user_confirmation = data.get('user_confirmation')
        if not user_confirmation:
            return jsonify({"error": "User confirmation is required"}), 400
        ...
        if task.status != "awaiting_confirmation":
            return jsonify({"error": f"Task is not awaiting confirmation (current status: {task.status})"}), 400

        # Update task with confirmation
        task.user_confirmation_response = user_confirmation
        task.message = "Confirmation received. Generating final queries..."
        task.status = "pending"
        ...
        def run_async_task():
            ...
            asyncio.set_event_loop(loop)
            try:
                loop.run_until_complete(continue_task(task_id))
            finally:
                loop.close()
...
def task_results(task_id):
    ...
    if task.status not in ["completed", "failed"]:
        return jsonify({**task, "results": None})

    include_papers = request.args.get('include_papers', 'true').lower() == 'true'
    

    # Here you would typically load the actual results from files
    # For now, we'll return a mock response
    results_data = get_task_results(task_id,include_papers)
   
    task_dict = results_data.dict() if hasattr(results_data, 'dict') else vars(results_data)
    return jsonify(task_dict)
