import asyncio
from gptservice import GPT,gemini_mode_list
from deepseek_conversation import DeepseekConversation
import time

async def demo_gemini_single_round():
    """演示单轮对话的使用方法"""
    print("==================== 单轮对话示例 ====================")
    startTime = time.time()
    # 使用原有模型
    messages = [{"role": "user", "content": "证明素数有无数个。"}]
    response = await GPT(messages, selected_model='gemini-2.5-pro-exp-03-25')
    endTime = time.time()
    print(f"\n原始模型 (gemini-2.5-pro-exp-03-25) 回复: {response['content']}")
    print(f'Use Tick:{endTime - startTime}')
...
async def main():
    ...
    #await gemini_mode_list()
    
    await demo_gemini_single_round()
    # 运行单轮对话示例
